#' Generate Menstrual Survey Query
#'
#' This function generates a SQL query for Menstrual Survey data based on the specified tier for BQ2 population(production, staging, or development).
...
#'
#' @return A SQL query string for fetching Menstrual Survey data is written to an output file named 'menstrualsurvey.sql' in the corresponding tier directory.
#'
#' @details 
...
#'
#' @examples
#' generate_menstrualsurvey_query("prod")
#'
#' @author Jing Wu, Jake Peters, Rebecca Sansale
#'
generate_menstrualsurvey_query <- function(tier){
...
#get the DD
#"current text CID" is used to match with schema last CID values
...
#pull schema
table_name <- "menstrualSurvey_v1_JP"
sql <- glue("SELECT * 
...
sql    <- glue(
   ...
   outputpath = glue("sql/{tier}/menstrualsurvey.sql")
