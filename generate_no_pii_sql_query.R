generate_nonpii_sql_query <- function(project, tier, table){
  ...
  dd <- read.csv(dd_url) 
  dd$concept_id_primary_source <- dd$conceptId
  dd$concept_id_secondary_source <- dd$conceptId.1
  dd$concept_id_source_question <- dd$conceptId.2
  dd$concept_id_question_text <- dd$conceptId.3
  dd$concept_id_response <- dd$conceptId.4
  ...
  non_pii_variables <- dd[dd$PII=="No",]#I only want the NON PII variables
  non_pii_variables <- non_pii_variables[non_pii_variables$Deprecated..New..or.Revised!= "Deprecated",]$concept_id_question_text
...
#  table_non_pii_variables <- c(paste0("t.", table_non_pii_variables), "t.Connect_ID") #SQL join syntax
  ...
  write.table(select,file=paste("/Users/sansalerj/Desktop/bq2/bq2/updated_bq2_query_gen/", table, ".txt", sep=""), row.names = FALSE, col.names = FALSE, quote = FALSE)
